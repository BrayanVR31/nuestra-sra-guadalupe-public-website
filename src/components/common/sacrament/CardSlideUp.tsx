...
export default function CardSlideUp({ image, label, color, overlayColor, foreground, path }: CardSlideUpProps) {
  return (
    <motion.article
      variants={variants}
      ...
      <figcaption className="absolute inset-0 flex flex-col items-center justify-center px-8">
        ...
        <div className="relative w-full overflow-hidden text-center">
          ...
          <h3 style={{ backgroundColor: `rgb(from ${color} r g b / 0.55)`, color: foreground }} className="absolute top-0 left-0 w-full py-3 text-lg font-extrabold uppercase tracking-widest transition-transform duration-300 ease-out group-hover:-translate-y-full text-shadow-lg/40">
            {label}
          </h3>
        </div>
      </figcaption>
